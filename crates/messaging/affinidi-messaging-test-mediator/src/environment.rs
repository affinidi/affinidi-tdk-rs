...
impl TestEnvironment {
    ...
    pub async fn spawn_ungated_with_tsp_policy(
        ...
        // Direct delivery on, for the same reason as `spawn_with_tsp_policy`:
        // `local_direct_delivery_allowed` refuses a local direct delivery
        // before any of these tests reach their subject. This helper was
        // written before that gate existed, so it did not need the flag then.
        let mediator = TestMediator::builder()
            .local_direct_delivery(true, false)
            .spawn()
            .await?;
