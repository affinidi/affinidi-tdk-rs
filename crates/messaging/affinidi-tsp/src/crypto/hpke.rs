...
pub fn seal(
    ...
    seal_with_ephemeral(plaintext, aad, recipient_pk, info, sk_e)
}

/// [`seal`] with the ephemeral key derived from `ikm_e` — RFC 9180's
/// `DeriveKeyPair(ikmE)` — instead of drawn fresh.
///
/// Crate-private, and reachable from outside only through the `test-vectors`
/// feature. It exists because the specification's Appendix A publishes `ikmE`
/// so a vector can be regenerated byte for byte. Everything [`seal`] says
/// about nonce safety rests on the ephemeral key being unique per message:
/// two seals under the same `ikm_e` to the same recipient share the key and
/// the nonce, and reveal the XOR of their plaintexts.
pub(crate) fn seal_with_ikm_e(
    plaintext: &[u8],
    aad: &[u8],
    recipient_pk: &[u8; 32],
    info: &[u8],
    ...
    seal_with_ephemeral(plaintext, aad, recipient_pk, info, derive_key_pair(ikm_e)?)
}
...
fn seal_with_ephemeral(
    ...
    let (shared_secret, enc) = encap(&sk_e, &pk_r)?;
...
///
/// 1. Take the ephemeral X25519 keypair (fresh per message; see [`seal`])
...
fn encap(sk_e: &StaticSecret, pk_r: &PublicKey) -> Result<([u8; N_SECRET], [u8; 32]), TspError> {
...
/// DeriveKeyPair for DHKEM(X25519, HKDF-SHA256) (RFC 9180 §7.1.3).
///
/// `dkp_prk = LabeledExtract("", "dkp_prk", ikm)` and then
/// `sk = LabeledExpand(dkp_prk, "sk", "", Nsk)`. X25519 needs no rejection
/// sampling: every 32-byte string is a scalar once clamped, and the clamping
/// happens inside the DH.
fn derive_key_pair(ikm: &[u8; 32]) -> Result<StaticSecret, TspError> {
    let mut dkp_prk = labeled_extract(KEM_SUITE_ID, &[], b"dkp_prk", ikm)?;
    let mut sk = [0u8; 32];
    labeled_expand(KEM_SUITE_ID, &dkp_prk, b"sk", &[], &mut sk)?;
    dkp_prk.zeroize();
...
mod tests {
    ...
    /// RFC 9180 Appendix A.2 again, from the sender's side: `ikmE` derives
    /// the published `pkEm` (which is `enc`), and sealing under it reproduces
    /// the published ciphertext exactly. Pins `DeriveKeyPair` and the seal
    /// path, which the known-answer test above cannot reach.
    #[test]
    fn rfc9180_a2_seal_from_ikm_e_known_answer() {
        let ikm_e = hex32("909a9b35d3dc4713a5e72a4da274b55d3d3821a37e5d099e74a647db583a904b");
        let sk_rm = hex32("8057991eef8f1f1af18f4a9491d16a1ce333f695d4db8e38da75975c4478e0fb");
        let pk_rm = PublicKey::from(&StaticSecret::from(sk_rm)).to_bytes();
        let info = hex::decode("4f6465206f6e2061204772656369616e2055726e").unwrap();
        let aad = hex::decode("436f756e742d30").unwrap();
        let pt = hex::decode("4265617574792069732074727574682c20747275746820626561757479").unwrap();

        let sealed = seal_with_ikm_e(&pt, &aad, &pk_rm, &info, &ikm_e).unwrap();
        assert_eq!(
