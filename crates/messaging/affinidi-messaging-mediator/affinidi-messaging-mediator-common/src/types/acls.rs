...
/// Failures originating in the ACL parser / mutator code paths. The
/// SDK wraps this in `ATMError`; the mediator wraps it in
...
pub enum ACLError {
    ...
    Config(String),
    /// Caller does not have permission to perform the requested change.
    #[error("ACL denied: {0}")]
    Denied(String),
}
...
impl MediatorACLSet {
    ...
    pub fn from_string_ruleset(input: &str) -> Result<Self, ACLError> {
        ...
        for item in input.split(',') {
            ...
            match item {
                ...
                _ => {
                    return Err(ACLError::Config(format!("Invalid ACL String ({item})")));
                }
            }
        }
    ...
    pub fn from_hex_string(hex: &str) -> Result<MediatorACLSet, ACLError> {
        let acl = u64::from_str_radix(hex, 16)
            .map_err(|_| ACLError::Config(format!("Invalid ACL Hex String ({hex})")))?;
