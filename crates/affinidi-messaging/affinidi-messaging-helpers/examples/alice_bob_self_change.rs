...
async fn main() -> Result<(), ATMError> {
    ...
    println!("Alice streaming response from Mediator");
    match protocols
        .message_pickup
        .live_stream_next(&atm, &atm_alice, Some(Duration::from_secs(10)), true)
        .await?
    {
        Some(msg) => {
            println!();
            println!(
                "Decrypted Message from Mediator to Alice msg_id({}):\n {:#?}\n",
                msg_id, msg.0
            );
        }
        None => {
